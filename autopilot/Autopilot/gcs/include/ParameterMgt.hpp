...
protected:
	typedef struct
	{
		...
		uint8_t count; // Number of parameters currently saved in eeprom (to be compared with Flash)
		uint8_t crc; // CRC of data in EEPROM
	} Header;

public:
	ParameterMgt(const ParamInfo* info, uint8_t paramCount);
	virtual ~ParameterMgt();

	bool find(const char* name, uint8_t& idx);
	inline bool getInfo(uint8_t idx, enum MAV_PARAM_TYPE* type, char* name=NULL);

	/** @brief Read parameter value */
	bool read(uint8_t idx, Value& value);

	/** @brief Write parameter value */
	bool write(uint8_t idx, const Value& value);

	/** @brief Get parameter count */
	inline uint8_t getParamCount();
...
protected: /* REGULAR METHODS */
	...
	bool getFullInfo(uint8_t idx, enum MAV_PARAM_TYPE* type, char* name=NULL, void** currentValue=NULL, Value* eepromValue=NULL);

	/* Save current value into EEPROM */
	bool save(uint8_t paramIdx, bool updateCrc = false);
	/* Load current value from EEPROM */
	bool load(uint8_t paramIdx);
	/* Load current value from program memory */
	bool loadDefault(uint8_t paramIdx);
...
inline bool ParameterMgt::getInfo(uint8_t idx, enum MAV_PARAM_TYPE* type, char* name)
{
	return getFullInfo(idx, type, name, NULL, NULL);
}
inline uint8_t ParameterMgt::getParamCount()
{
	return paramCount;
}
